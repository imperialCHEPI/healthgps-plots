...
if (length(commandArgs(trailingOnly = TRUE)) < 1) {
  stop("Usage: Rscript script.R <path1> [<path2> <path3> ...] or Rscript script.R <directory>")
}
...
if (file.info(arg1)$isdir) {
  ...
  files <- list.files(arg1, pattern = "\\.csv$", full.names = TRUE)
  data_list <- map(files, read.csv)
  data <- do.call(rbind, data_list)
} else {
  # If it's not a directory, treat the arguments as individual file paths
  paths <- commandArgs(trailingOnly = TRUE)
  data_list <- map(paths, read.csv)
  data <- do.call(rbind, data_list)
}
